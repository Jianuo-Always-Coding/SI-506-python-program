...
print('\n1.4.2 Ch 01 Tesla EVs (str.startswith()')
for vehicle in elec_vehicles:
    if vehicle[0].lower().startswith('tesla'):
        print(vehicle)

# Alternative (str.find())
print('\n1.4.3 Ch 01 Tesla EVs (str.find()')
for vehicle in elec_vehicles:
...
for vehicle in elec_vehicles:
    if int(vehicle[-1]) >= 250:
        vehicle_count += 1 # equivalent to mpg_count = mpg_count + 1
...
for i in range(len(elec_vehicles)):
    elec_vehicles[i][1] = f"{elec_vehicles[i][0]} {elec_vehicles[i][1]}"
